...
class Species(Resource):
    ...
    def __init__(self, data):
        ...
        self.genderRatio: float = data.get(
            "gender_rate"
        )  # "Chance of this pokemon being female, in eighths. -1 if genderless"
    ...
    def CaptureRate(self) -> str:
        ...
        captureRate = float(
            (
                (1 + (maxHP * 3 - maxHP * 2) * self.catchRate * ballRate * status)
                / (maxHP * 3)
            )
            / 256
        )
        ...
        return str(
            f"{self.catchRate} (~{(captureRate * 100):.2f}% in PokeBall @ full HP)"
        )
    ...
    def PrintDataForPokemonPage(self):
        ...
        infoTable.add_row(
            "[b]Egg Groups:[/]", self.EggGroups, "[b]Hatch Cycles:[/]", self.HatchCycles
        )
