...
class TestCacheManager:
    ...
    def clean_cache_manager(self):
        saved_mappings = CacheManager.cache_mappings.copy()
        CacheManager.cache_mappings = {}

        yield

        CacheManager.cache_mappings = saved_mappings
    ...
    def test_add_name_to_ID_mapping(self, clean_cache_manager):
        ...
        assert CacheManager.cache_mappings.get(test_key) is not None
        assert (
            CacheManager.cache_mappings.get(test_key).NAME_TO_ID.get(test_name)
            == test_ID
        )
        assert (
            CacheManager.cache_mappings.get(test_key).NAME_TO_ID.get(bad_name) is None
        )
    ...
    def test_add_ID_to_data_mapping(self, clean_cache_manager):
        ...
        assert CacheManager.cache_mappings.get(test_key) is not None
        assert (
            CacheManager.cache_mappings.get(test_key).ID_TO_DATA.get(test_ID)
            == test_data
        )
        assert CacheManager.cache_mappings.get(test_key).ID_TO_DATA.get(bad_ID) is None
